...
genome2completeness = pd.read_csv(
    '/mnt/ivy/thliao/project/AOB/analysis/20210713_repeat/xmoA_genome_tree/quality/micomplete_o/bac_output.completeness', comment='#', sep='\t', index_col=0)
genome2completeness = genome2completeness['Weighted completeness'].to_dict()
genome2completeness['GCF_9000785151'] = 0.7121

stree = "/mnt/ivy/thliao/project/AOB/analysis/20210713_repeat/xmoA_genome_tree/phylo_20220926/xmoA_containing.MVrooted_named.newick"
st = Tree(stree,3)
def select_repr_from_clusters(g):
    repr_g = sorted(g, key=lambda x: genome2completeness[x.replace('.','')])[-1]
    return repr_g
...
for a in [beta_amo, nitro_amo, gamma_amo, pmo_lineage]:
    ...
    dis, idx = nbrs.kneighbors(dm_df.loc[gids, :].mean().values.reshape(1, -1),
